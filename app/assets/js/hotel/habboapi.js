...
function () {
	...
	window.FlashExternalInterface = {}, window.FlashExternalInterface.closeHabblet = function (n) {
		...
		notify(null, "Robot Billy", "Ta connexion au serveur de HabboCity perd de sa stabilité, il se peut qu'elle redevienne normale dans quelques instants, mais il se peut aussi que tu sois déconnecté.");
		window.MainApp.postMessage({
			call: "disconnect"
		})
		...
		var message;
		console.log(a);
		if (a.substring(0, 6) === "/alert") {
			message = decodeMessage(a, "/alert?");
			console.log(message);
			newAlert(message.type, nl2br(message.description), message.header, message.title, message.username, message.figure, message.button_text, message.button_event);
		} else if (a.substring(0, 10) === "/quickpoll") {
			message = decodeMessage(a, "/quickpoll?");
			if (message.type === "start") {
				startQuickPoll(message.question);
			} else if (message.type === "end") {
				endQuickPoll();
			} else if (message.type === "updatevote") {
				updateQuickPoll(message.yesvotes, message.novotes);
			}
		} else if (a.substring(0, 12) === "/buildAction") {
			message = decodeMessage(a, "/buildAction?");
			if (message.type === "start") {
				$("#buildmode").show();
			} else {
				$("#buildmode").hide();
			}
		} else if (a.substring(0, 9) === "/roomsale") {
			message = decodeMessage(a, "/roomsale?");
			if (message.type === "open") {
				showVendreMarche();
				setVendreMarche(message.name, message.id, message.thumbnail, message.description);
				actionVendreMarche();
			} else {
				hideVendreMarche();
			}
		} else if (a.substring(0, 9) === "/calendar") {
			openCalendar();
		} else if (a.substring(0, 8) === "/players") {
			message = decodeMessage(a, "/players?");
			updatePlayers(message);
		} else if (a.substring(0, 10) === "/bienvenue") {
			openBienvenue();
		} else if (a.substring(0, 12) === "/cityclub") {
			opennCityClub()
		} else if (a.substring(0, 8) === "/mention") {
			message = decodeMessage(a, "/mention?");
			notify(message.room, message.username, message.message);
		} else if (a.substring(0, 6) === "/robot") {
			message = decodeMessage(a, "/robot?");
			notify(null, message.username, message.message);
		} else if (a.substring(0, 16) === "/diamantsbooster") {
			message = decodeMessage(a, "/diamantsbooster?");
			if (message.type === "start") {
				startDiamondsBooster(message.value);
			} else if (message.type === "end") {
				endDiamondsBooster();
			} else if (message.type === "update") {
				updateDiamondsBooster(message.value);
			}
		} else {
		...
		console.log("Je viens de changer de look");
		window.MainApp.postMessage({
			call: "update-figure",
			target: n
		})
		...
		if (n.data) {
			...
			switch (t.call) {
				...
				case "interstitial-status":
					o(t.target)
				case "disconnect":
					notify(null, "Robot Billy", "J'ai un peu de mal à joindre le serveur, si tu n'arrives plus ou pas à jouer, actualise la page.");
			}
		}
	}
...
function decodeMessage(msg, key) {
	var params = (msg.replace(key, "")).split("&");
	var message = {};
	for (var i = 0; i < params.length; i++) {
		params[i] = params[i].split("=");
		message[decodeString(params[i][0])] = decodeString(params[i][1]);
	}
	return message;
}
...
function writeMessage(message) {
	flashEvent.openlink("navigator/goto/" + window.btoa(unescape(encodeURIComponent(message))))
}
